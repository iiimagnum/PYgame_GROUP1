    ...
    while True:
        ...
        while running and retry:
            ...
            while running and retry:
                ...
                pygame.display.flip()
        if retry == False:
            ffont = pygame.font.SysFont(fonts[0], 100)
            ffont.bold = True
            fffont = pygame.font.SysFont(fonts[0], 50)
            fffont.bold = True
            textSurface = ffont.render("Game Over", True, (255, 255, 255))
            MainSurface.blit(textSurface, (100, 100))
            textSurface = fffont.render("Press R to retry", True, (255, 255, 255))
            MainSurface.blit(textSurface, (100, 300))
            textSurface = fffont.render("press Q or Esc to quit", True, (255, 255, 255))
            MainSurface.blit(textSurface, (100, 400))
            pygame.display.flip()
            while True:
                InputManager.update()
                if InputManager.quit:
                    pygame.quit()
                if InputManager.keyDownList.__contains__(pygame.K_ESCAPE) or InputManager.keyDownList.__contains__(
                        pygame.K_q):
                    pygame.quit()
                if pygame.K_r in InputManager.keyDownList:
                    retry = True
                    break
